...
export var StateMachine = {
    ...
    create: function (cfg, target) {
        ...
        fsm.can = function (event) {
            return !this.transition && (map[event].hasOwnProperty(this.current) || map[event].hasOwnProperty(
    ...
    doCallback: function (fsm, func, name, from, to, args) {
        if (func) {
            try {
                ...
                return fsm.error(name, from, to, args, StateMachine.Error.INVALID_CALLBACK,
    ...
    buildEvent: function (name, map) {
        return function () {
            ...
            if (this.transition)
                return this.error(name, from, to, args, StateMachine.Error.PENDING_TRANSITION, "event " +
                    name + " inappropriate because previous transition did not complete");

            if (this.cannot(name))
                return this.error(name, from, to, args, StateMachine.Error.INVALID_TRANSITION, "event " +
                    name + " inappropriate in current state " + this.current);
